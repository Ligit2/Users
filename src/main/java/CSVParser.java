...
public class CSVParser {
    public static Map<Character, List<Person>> parse() {
        ...
        try {
            CSVReader reader =
                    new CSVReaderBuilder(new FileReader("C:\\Users\\User\\IdeaProjects\\MyApp\\AllUsers.csv")).
                            withSkipLines(1).
                            build();
            collect = reader.readAll().stream().map(data -> {
                Person person = new Person(data[0], data[1], data[2], data[3], data[4], data[5]);
                return person;
            }).sorted(new ArithmeticComparator()).collect(Collectors.groupingBy(data -> data.getSurname().charAt(0)));
